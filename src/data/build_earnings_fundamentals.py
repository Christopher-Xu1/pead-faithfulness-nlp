...
def load_external_earnings_events(path: str | Path | None) -> pd.DataFrame | None:
    if path is None:
        return None
    csv_path = Path(path)
    ...
    standardized = out[["ticker", "event_date"]].copy()
...
def build_earnings_fundamentals(
    ...
    if external_events_df is not None and not external_events_df.empty:
        out = out.merge(external_events_df, on=["ticker", "event_date"], how="left")
    else:
